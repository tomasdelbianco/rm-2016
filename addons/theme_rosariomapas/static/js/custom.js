...
// Set width for inputs in horizontal search bar -----------------------------------------------------------------------
    ...
    $( "#redefine-search-form" ).load( "/theme_rosariomapas/static/external/_search-bar.html", function() {
        setInputsWidth();
        //autoComplete();
    });
...
//  iCheck -------------------------------------------------------------------------------------------------------------
    ...
    $('a').on('click', function (e) {
        ...
        if (typeof attrs != "undefined" && attr.indexOf('#') != 0 ) {
            e.preventDefault();
...
function autoComplete(){
    if( !$("script[src='/theme_rosariomapas/static/js/leaflet.js']").length ){
...
function drawOwlCarousel(_rtl){
    $.getScript( "/theme_rosariomapas/static/js/owl.carousel.min.js", function( data, textStatus, jqxhr ) {
        ...
        if( category == 'real_estate' ){
            if( json.data[a].item_specific ){
                if( json.data[a].item_specific.bedrooms ){
                    itemSpecific += '<span title="Bedrooms"><img src="/theme_rosariomapas/static/img/bedrooms.png">' + json.data[a].item_specific.bedrooms + '</span>';
                }
                if( json.data[a].item_specific.bathrooms ){
                    itemSpecific += '<span title="Bathrooms"><img src="/theme_rosariomapas/static/img/bathrooms.png">' + json.data[a].item_specific.bathrooms + '</span>';
                }
                if( json.data[a].item_specific.area ){
                    itemSpecific += '<span title="Area"><img src="/theme_rosariomapas/static/img/area.png">' + json.data[a].item_specific.area + '<sup>2</sup></span>';
                }
                if( json.data[a].item_specific.garages ){
                    itemSpecific += '<span title="Garages"><img src="/theme_rosariomapas/static/img/garages.png">' + json.data[a].item_specific.garages + '</span>';
                }
                return itemSpecific;
            }
        }
...
function quickView(id){
    $.ajax({
        type: 'POST',
        url: '/theme_rosariomapas/static/external/_modal.html',
        data: id,
